...
These used to be absolute paths under one developer's home directory, which
made every Blender render in `scripts/video/` unrunnable on any other machine.
...
The renders specify `BLENDER_EEVEE_NEXT` and are engine-version-sensitive, so
point `$BLENDER` at a 5.0.x build rather than falling back to an older one.
...
# Resolved lazily by find_blender(); module import must not fail on a machine
# without Blender, since some of these scripts have non-Blender code paths.
BLENDER = (
