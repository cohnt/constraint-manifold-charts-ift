...
def shortcut(path, ValidityChecker, num_tries=100, Distance=None, check_size=1e-1, EdgeValidator=None):
	if Distance is None:
	...
	for _ in tqdm(range(int(num_tries))):
		...
		for step_count in np.arange(1, 1+int(dist / check_size)):
			q_chk = q1 + validity_step * step_count
			if not ValidityChecker(q_chk):
				add = False
				break
			if EdgeValidator and not EdgeValidator(prev_chk, q_chk):
				add = False
				break
			prev_chk = q_chk
		if add and EdgeValidator and not EdgeValidator(prev_chk, q2):
			add = False
		if add:
