"""
render_grasp_figure.py

Render the grasp-selection figure in Blender from a Meshcat scene exported by
scripts/visualize_grasp_selection.py.

This runs inside Blender's own Python, not the project venv:

    blender --background --python scripts/render_grasp_figure.py -- \
        --html out/grasp_selection.html --out out/grasp_selection.png

Use scripts/render_grasp_figure.sh, which fills in the Blender path and defaults.

Transparency is the point of the figure: the three arms are drawn semi-transparent so
all of them read at once.  Drake stamps that alpha into the illustration properties,
Meshcat carries it as material opacity, and the meshcat_html_importer maps it onto the
Principled BSDF's Alpha input -- so the alpha arrives with the geometry and this script
must not overwrite it.  It only verifies the alpha survived, and reports if it did not.
"""
...
# Orbited to azimuth 95 deg, elevation 25 deg about the mug.  The three arms converge
# from the left here, leaving a clear window onto the mug; from further left (lower
# azimuth) a wrist link crosses in front of it, and from directly overhead the arms
# stack over it entirely.
DEFAULT_CAMERA_POS = (0.304, 0.872, 0.528)
DEFAULT_CAMERA_TARGET = (0.38, 0.0, 0.12)
...
def parse_args():
    ...
    ap.add_argument("--arm-alpha", type=float, default=None,
                    help="Override the arm transparency used when restyling (0-1).")
    ...
    ap.add_argument("--frame-radius", type=float, default=0.30,
                    help="Radius in metres around the target to fit in frame. Smaller = "
                         "tighter crop on the grasp.")
...
def report_transparency():
    """The figure is meaningless if the arms come in opaque, so check and report."""
    transparent, opaque = [], []
    ...
    if not transparent:
        print("[materials] WARNING: nothing is transparent -- the imported scene lost its "
              "alpha, so the overlapping arms will not read correctly.")
    else:
...
MATERIALS = {
    "ur5e":       ((0.55, 0.56, 0.58), 0.42, 0.05, 0.50),   # fallback only; see above
    "wsg":        ((0.16, 0.17, 0.19), 0.55, 0.85, 0.30),   # Schunk dark anodised body
...
def owning_model(obj):
    ...
    for collection in obj.users_collection:
        ...
        for name in chain:
            base = name.split(".")[0]
            if base in TABLE_MODELS:
                return base
            for model in ("ur5e", "wsg", "target_mug"):
                if base == model or base.startswith(model + "_"):
                    return model
    return None
...
def restyle_scene(arm_alpha=None, wood_table=True):
    ...
    for obj in bpy.data.objects:
        ...
        if FINGER_MATERIAL_HINT in existing:
            counts["finray (kept)"] = counts.get("finray (kept)", 0) + 1
            continue
...
def setup_lighting(target):
    """Three-point lighting aimed at the grasp, plus a soft ambient world."""
    ...
    focus = Vector(target)
    # (name, offset from focus, power W, radius m)
    lights = [
...
def main():
    ...
    setup_lighting(args.camera_target)
